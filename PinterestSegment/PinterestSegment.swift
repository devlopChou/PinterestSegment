...
public struct PinterestSegmentStyle {
    ...
    public var defaultColor = UIColor(white: 0.95, alpha: 1)
...
@IBDesignable public class PinterestSegment: UIControl {
    ...
    private let unselectContent = UIView()

    private var unindicator: UIView = {

        let ind = UIView()

        ind.backgroundColor = .blue
    ...
    private func setunIndicatorFrame(_ frame: CGRect) {
    ...
    private func reloadData(animated: Bool = true, sendAction: Bool = true) {
        ...
        for (index, item) in titleElements.enumerated() {
            ...
            let rect2 = CGRect(x: titleX, y: coverY, width: titleW, height: coverH)
            ...
            backLabel.frame = rect2

            

            backLabel.layer.cornerRadius = backLabel.frame.size.height / 2

            backLabel.layer.backgroundColor = style.defaultColor.cgColor

            

            if style.normalBorderColor != .clear {

                backLabel.layer.borderColor = self.normalBorderColor.cgColor
